...
/*!
    ...
    @copyright 2017-2020 by Claude Heintz
        ...
        Test of LXTeensy4DMX RDM functions
...
void identifyEach() {
  ...
  while ( notDone ) {
    ...
    if ( i < 0 ) {
      ...
      uint8_t data[2];
      if ( Teensy4DMX.sendRDMGetCommand(found, RDM_DEVICE_START_ADDR, data, 2) ) {
        uint16_t addr = (data[0] << 8) | data[1];

        if ( addr == 0x0F ) {
          data[0] = 0x00;
          data[1] = 0x01;
          Teensy4DMX.sendRDMSetCommand(found, RDM_DEVICE_START_ADDR, (uint8_t*)data, 2);
        }
  
        data[0] = 0x01;
        Teensy4DMX.sendRDMSetCommand(found, RDM_IDENTIFY_DEVICE, (uint8_t*)data, 1);
        delay(2000);
        data[0] = 0x00;
        Teensy4DMX.sendRDMSetCommand(found, RDM_IDENTIFY_DEVICE, (uint8_t*)data, 1);
      }
...
void setup() {
  ...
  Teensy4DMX.startRDM(DIRECTION_PIN, RDM_DIRECTION_OUTPUT, RX_SIGNAL_INVERTED);
...
void loop() {
  ...
  Teensy4DMX.setSlot(7,testLevel);
  Teensy4DMX.setSlot(8,255);
  Teensy4DMX.setSlot(371,testLevel);
  Teensy4DMX.setSlot(22,255);
  loopDivider++;
  if ( loopDivider == 4 ) {
    testLevel++;
    loopDivider = 0;
