...
/*!
    ...
    This sketch allows a Teensy 4 board to emulate parts of an ENTTEC DMX USB Pro's functions.
    Instead of using Teensy 4's USB Serial, this version uses TX3/RX3 and requires an external
...
Pins 0 & 1 are UART0 Rx/Tx which is used by LXTeensy4DMX.
...
void setup() {
  ...
  Teensy4DMX.setDirectionPin(RXTX_PIN); 
...
void doOutputMode() {
  ...
  if ( label == ENTTEC_LABEL_SEND_DMX ) {
    ...
    for(int i=0; i<s; i++) {
      Teensy4DMX.setSlot(i,eSerial.getSlot(i));
    }
    Teensy4DMX.startOutput();  //ignored if already started
    setLED(GRN_LED_BIT);
...
void doInputMode() {
  if ( Serial3.available()) {  //writing anything to the USB switched to output mode
	Teensy4DMX.setDataReceivedCallback(0);
  ...
  if ( got_dmx ) {
    int msg_size = got_dmx;
	 eSerial.writeDMXPacket(Teensy4DMX.dmxData(), msg_size);
    got_dmx = 0;
  }
