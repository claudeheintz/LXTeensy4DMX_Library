...
/*!
    ...
    This sketch allows a Teensy 4 board to emulate some functions
...
void doOutputMode() {
  ...
  if (( label == ENTTEC_LABEL_SEND_DMX ) || ( label == ENTTEC_LABEL_SEND_DMX_ULTRA)) {
    ...
    for(int i=0; i<s; i++) {
      Teensy4DMX1.setSlot(i,eSerial.getSlot2(i));
    }
    Teensy4DMX1.startOutput();  //ignored if already started
    setLED(BLU_LED_BIT);
