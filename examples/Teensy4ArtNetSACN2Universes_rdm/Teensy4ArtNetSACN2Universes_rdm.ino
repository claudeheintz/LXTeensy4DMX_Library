/**************************************************************************/
/*!
    @file     Teensy4ArtNetSACN2Universes.ino
    @author   Claude Heintz
    @license  BSD (see LXDMXEthernet.h or http://www.claudeheintzdesign.com/lx/opensource.html
    @copyright 2017-2020 by Claude Heintz All Rights Reserved

    Example using LXEDMXEthernet_Library for 2 universes of DMX output using
    Teensy4 with a wiznet based shield to receve Art-Net or sACN e1.31
    
    Art-Net(TM) Designed by and Copyright Artistic Licence Holdings Ltd.
    sACN E 1.31 is a public standard published by the PLASA technical standards program
    
    Note:  This example requires the LXTeensy4DMX_Library for DMX serial output
...
void artRDMReceived(uint8_t* pdata) {
  ...
  uint8_t* pkt = Teensy4DMX.rdmPacket();
  for (j=0; j<plen; j++) {
    pkt[j+1] = pdata[j];
  }
  pkt[0] = 0xCC;

  if ( Teensy4DMX.sendRDMControllerPacket() ) {
    artNetInterface->send_art_rdm(&aUDP, Teensy4DMX.rdmData(), aUDP.remoteIP());
  }
...
uint8_t testMute(UID u) {
   ...
   if ( Teensy4DMX.sendRDMDiscoveryMute(u, RDM_DISC_MUTE) ) {
     return 1;
   }
   if ( Teensy4DMX.sendRDMDiscoveryMute(u, RDM_DISC_MUTE) ) {
     return 1;
...
uint8_t checkTable(uint8_t ck_index) {
  if ( ck_index == 0 ) {
    Teensy4DMX.sendRDMDiscoveryMute(BROADCAST_ALL_DEVICES_ID, RDM_DISC_UNMUTE);
  }
  ...
  if ( discoveryTree.pop(&upper) ) {
    if ( discoveryTree.pop(&lower) ) {
      if ( lower == upper ) {
        ...
        uint8_t result = Teensy4DMX.sendRDMDiscoveryPacket(lower, upper, &found);
        if ( result ) {
          //this range responded, so divide into sub ranges push them on stack to be further checked
          pushActiveBranch(lower, upper);
           
        } else if ( Teensy4DMX.sendRDMDiscoveryPacket(lower, upper, &found) ) {
...
#warning PRINT_DEBUG_MESSAGES is defined
  ...
  strcpy((char*)&pollReply[26], "Teensy4DMX");
  strcpy((char*)&pollReply[44], "Teensy4DMX");
...
// multicast requires modificaton of Ethernet library.  see LXDMXEthernet.h
  ...
  Teensy4DMX.setDirectionPin(U1_DIR);
  
  if ( rdm_enabled ) {
  	Teensy4DMX.startRDM(U1_DIR, RDM_DIRECTION_OUTPUT, RX_SIGNAL_NORMAL);
  } else {
    Teensy4DMX.startOutput();
  }
  Teensy4DMX2.startOutput();
...
void copyDMX1ToOutput(void) {
  ...
  for (int i=1; i <=DMX_UNIVERSE_SIZE; i++) {
    ...
    if ( a > s ) {
        Teensy4DMX.setSlot(i , a);
      } else {
        Teensy4DMX.setSlot(i , s);
      }
   }
}
...
void copyDMX2ToOutput(void) {
  ...
  for (int i=1; i <=DMX_UNIVERSE_SIZE; i++) {
    ...
    if ( a > s ) {
        Teensy4DMX2.setSlot(i , a);
      } else {
        Teensy4DMX2.setSlot(i , s);
      }
   }
}
